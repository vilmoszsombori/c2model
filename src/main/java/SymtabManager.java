...
public class SymtabManager {
    ...
    /**
     * Stack of scopes. Currently max. nesting allowed is 255.
     */
    static Scope[] scopeStack = new Scope[255];
     ...
     */
    public static Scope OpenScope(String scopeName, int type) {
        Scope newScope;
        ...
        if (scopeName != null) {
            if (type == Scope.TYPE) {
                ...
                newScope = new Scope(scopeName, type, scopeStack[depth]);
            }
    ...
    public static void PutVariable(Variable var) {
    	scopeStack[depth].PutVariable(var);
    }
    
    public static void PutParameter(Variable var) {
    	scopeStack[depth].PutParameter(var);
    }        
    
    public static void PutFunction(String name, Scope sc) {
    	GetGlobalScope().PutFunction(name, sc);
    }
    
    public static void PutPort(String name, Integer t) {
    	Scope l = GetCurScope();
    	if(l.IsVariable(name))
    		while(!l.IsLocalVariable(name)) {
        		l.PutPort(name, t);
        		l = l.parent;    			
    		}
    }
    ...
    public static boolean IsGlobalVariable(String name) {
    	return GetGlobalScope().GetVariable(name) != null;
    }
    
    public static boolean IsVariable(String name) {
    	return GetCurScope().IsVariable(name);
    }
    ...
    public static Scope GetCurFunction() {
    	//System.out.println("returned as curfunction: " + scopeStack[2].scopeName);
    	return scopeStack[2];
    }
